...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter  {
    ...
    private void updateWearable() {
        ...
        Cursor data = getContext().getContentResolver().query(weatherForLocationUri, NOTIFY_WEATHER_PROJECTION, null,
        ...
        data.close();
        sendDataToWearable(iconId,maxTemp,minTemp);
    }
    ...
    private void notifyWeather() {
        ...
        if ( displayNotifications ) {
            ...
            if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
                Log.d(LOG_TAG,"Time elapsed" + (System.currentTimeMillis()-lastSync));
    ...
    private void sendDataToWearable(int bitmapId,double high,double low) {

        Log.d(LOG_TAG,"sendDataToWearable");
        googleApiClient = new GoogleApiClient.Builder(getContext())
                .addApi(Wearable.API)
                .build();
        ConnectionResult result = googleApiClient.blockingConnect(1000, TimeUnit.MILLISECONDS);
        if(!result.isSuccess()) {
            Log.d(LOG_TAG, "Failed to send bitmap to wearable");
            return;
        }

        Bitmap bitmap = BitmapFactory.decodeResource(getContext().getResources(), bitmapId);
        Asset asset = createAssetFromBitmap(bitmap);
        PutDataMapRequest dataMap = PutDataMapRequest.create("/wearable");
        ...
        DataApi.DataItemResult pResult = pendingResult.await();


        if(pResult.getStatus().isSuccess()) {
            Log.d(LOG_TAG, "Data item set: " + pResult.getDataItem().getUri());
        }
        else {
            Log.d(LOG_TAG, "Data item set failed");
        }
